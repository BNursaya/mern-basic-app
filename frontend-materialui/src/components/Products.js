...
const Products = () => {
  ...
  // ✅ Функцияны useCallback-пен тұрақтандырамыз
  ...
  }, [token]); // тек `token`-ге тәуелді

  // ✅ Енді қолдануға болады
  useEffect(() => {
    if (!loading) {
      if (!isAuthenticated) {
        navigate('/login');
  ...
  }, [isAuthenticated, loading, navigate, loadProducts]);
 ...
 const handleAddProduct = async (formData) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/products`, {
      ...
      headers: {
        Authorization: `Bearer ${token}`,
        // ❗ Content-Type көрсетпе — FormData өзі қояды
      },
      body: formData
    });
...
const handleUpdateProduct = async (id, name, price, imageFile = null) => {
  setIsLoading(true);
  try {
